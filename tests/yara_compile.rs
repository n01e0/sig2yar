...
#[test]
fn yara_rule_with_pcre_trigger_prefix_malformed_offset_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;200,abc:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    assert!(src.contains("pcre offset prefix '200,abc' unsupported"));
    assert_eq!(scan_match_count(src.as_str(), b"AAAAabc"), 0);
}

#[test]
fn yara_rule_with_pcre_trigger_prefix_missing_maxshift_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;200,:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    assert!(src.contains("pcre offset prefix '200,' unsupported"));
    assert_eq!(scan_match_count(src.as_str(), b"AAAAabc"), 0);
}

#[test]
fn yara_rule_with_pcre_trigger_prefix_missing_base_offset_false_rejects_scan() {
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_extra_comma_offset_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;200,300,400:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    assert!(src.contains("pcre offset prefix '200,300,400' unsupported"));
    assert_eq!(scan_match_count(src.as_str(), b"AAAAabc"), 0);
}

#[test]
fn yara_rule_with_pcre_trigger_prefix_empty_middle_offset_token_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;200,,300:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    assert!(src.contains("pcre offset prefix '200,,300' unsupported"));
    assert_eq!(scan_match_count(src.as_str(), b"AAAAabc"), 0);
}

#[test]
fn yara_rule_with_pcre_trigger_prefix_trailing_comma_offset_token_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;200,300,:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_double_leading_comma_offset_token_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;,,300:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_multi_empty_comma_offset_token_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;200,,,300:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_trailing_multi_empty_comma_offset_token_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;200,300,,:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_missing_base_and_trailing_comma_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;,300,:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_missing_base_and_trailing_multi_empty_comma_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;,300,,:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_double_leading_and_trailing_comma_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;,,300,:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_base_and_trailing_multi_empty_comma_without_maxshift_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;300,,:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_all_empty_comma_tokens_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;,,,:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    assert!(src.contains("pcre offset prefix ',,,' unsupported"));
    assert_eq!(scan_match_count(src.as_str(), b"AAAAabc"), 0);
}

#[test]
fn yara_rule_with_pcre_trigger_prefix_base_empty_middle_and_numeric_tail_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;300,,400:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    assert!(src.contains("pcre offset prefix '300,,400' unsupported"));
    assert_eq!(scan_match_count(src.as_str(), b"AAAAabc"), 0);
}

#[test]
fn yara_rule_with_pcre_trigger_prefix_missing_base_empty_middle_and_numeric_tail_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;,300,,400:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_base_multi_empty_middle_and_numeric_tail_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;300,,,400:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_missing_base_multi_empty_middle_and_numeric_tail_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;,300,,,400:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_comma_only_multi_empty_and_numeric_tail_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;,,,300:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_trigger_prefix_base_deep_empty_middle_and_numeric_tail_offset_token_false_rejects_scan(
) {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;300,,,,400:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
...
#[test]
fn yara_rule_with_pcre_ep_plus_range_without_e_matches_and_rejects_pe_fixture() {
    let data = pe_two_sections_fixture_with_aaaa_and_abc_in_section1();

    let sig_match =
        LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;EP+0,600:0/abc/").unwrap();
    let src_match = YaraRule::try_from(&sig_match).unwrap().to_string();
    assert!(src_match.contains("@s1[j] <= pe.entry_point + 0 + 600"));
    assert_eq!(scan_match_count(src_match.as_str(), &data), 1);

    let sig_nonmatch =
        LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;EP+0,100:0/abc/").unwrap();
    let src_nonmatch = YaraRule::try_from(&sig_nonmatch).unwrap().to_string();
    assert!(src_nonmatch.contains("@s1[j] <= pe.entry_point + 0 + 100"));
...
#[test]
fn yara_rule_with_pcre_anchored_offset_prefix_with_rolling_false_compiles_with_yara_x() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;10:0/abc/Ar").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    yara_x::compile(src.as_str())
        .expect("yara-x failed to compile anchored-offset-prefix+rolling safety-false rule");
    assert_eq!(scan_match_count(src.as_str(), b"abc"), 0);
}

#[test]
fn yara_rule_with_pcre_anchored_offset_prefix_with_re_flags_false_compiles_with_yara_x() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;10:0/abc/Are").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    yara_x::compile(src.as_str())
        .expect("yara-x failed to compile anchored-offset-prefix+re safety-false rule");
    assert_eq!(scan_match_count(src.as_str(), b"abc"), 0);
}
...
#[test]
fn yara_rule_with_pcre_section_end_offset_without_e_matches_pe_fixture() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;SE1:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();
    let data = pe_two_sections_fixture_with_aaaa_and_abc_in_section1();

    assert!(src.contains("@s1[j] >= pe.sections[1].raw_data_offset"));
    assert_eq!(scan_match_count(src.as_str(), &data), 1);
}
...
#[test]
fn yara_rule_with_pcre_macro_group_offset_prefix_without_closing_dollar_false_rejects_scan() {
...
#[test]
fn yara_rule_with_pcre_macro_group_offset_prefix_without_digits_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;$$:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    assert!(src.contains("pcre macro offset '$$' has invalid format"));
    assert_eq!(scan_match_count(src.as_str(), b"AAAAabc"), 0);
}
...
#[test]
fn yara_rule_with_pcre_out_of_range_macro_group_offset_prefix_false_rejects_scan() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:1;1;41414141;$32$:0/abc/").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    assert!(src.contains("pcre macro-group offset `$32$` out of range"));
    assert_eq!(scan_match_count(src.as_str(), b"AAAAabc"), 0);
}
...
#[test]
fn yara_rule_with_pcre_global_flag_matches_when_supported() {
    let sig = LogicalSignature::parse("Foo.Bar-1;Target:0;0&1;41414141;0/abc/g").unwrap();
    let rule = YaraRule::try_from(&sig).unwrap();
    let src = rule.to_string();

    yara_x::compile(src.as_str()).expect("yara-x failed to compile pcre-g generated rule");
...
#[test]
fn yara_rule_with_pcre_single_subsig_multi_gt_trigger_prefix_matches_and_rejects_scan() {
...
#[test]
fn yara_rule_with_pcre_single_subsig_multi_lt_trigger_prefix_matches_and_rejects_scan() {
