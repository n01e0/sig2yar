...
python3 - "${CORPUS_DIR}" "${CLAMAV_OUT}" "${YARA_HITS}" "${RULE_MAP}" "${EFFECTIVE_NAMES}" "${SUMMARY_JSON}" "${MISMATCH_TSV}" "${REPORT_MD}" <<'PY'
import collections
import json
import pathlib
import sys
...
if len(sys.argv) != 9:
    raise SystemExit("usage: <corpus_dir> <clamscan_out> <yara_hits> <rule_map> <effective_names> <summary_json> <mismatch_tsv> <report_md>")
...
summary_json = pathlib.Path(sys.argv[6])
mismatch_tsv = pathlib.Path(sys.argv[7])
report_md = pathlib.Path(sys.argv[8])
...
for rel in all_keys:
    ...
    if only_clamav or only_yara:
        mismatches.append((rel, only_clamav, only_yara))
...
summary = {
    ...
    "mismatch_examples": [
        ...
        for rel, oc, oy in mismatches[:20]
    ],
...
with mismatch_tsv.open("w", encoding="utf-8") as f:
    f.write("file\tonly_clamav\tonly_yara\n")
    for rel, oc, oy in mismatches:
        f.write(f"{rel}\t{','.join(oc)}\t{','.join(oy)}\n")
...
if summary["mismatch_examples"]:
    for item in summary["mismatch_examples"]:
        report_lines.append(
            f"- {item['file']} | only_clamav={item['only_clamav']} | only_yara={item['only_yara']}"
        )
else:
