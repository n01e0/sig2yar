...
- [ ] `byte_comparison` は `i`(raw, 1/2/4/8byte) と non-raw `=/>/< + exact(e)` を条件式にlower済み。unsupported ケース（non-rawの非exact/LE/表現不能値、rawの非1/2/4/8byte、矛盾した multi-clause）は safety false に倒す（fallbackではなく厳密化）。
