...
- [ ] `byte_comparison` は `i`(raw, 1/2/4/8byte) と non-raw `=/>/< + exact(e)` を条件式にlower済み。unsupported ケース（non-rawの非exact/LE/表現不能値、rawの非1/2/4/8byte・型幅超過閾値、矛盾した multi-clause）は safety false に倒す（fallbackではなく厳密化）。
...
- [ ] PCRE flags は `i/s/m/x/U/A` と ClamAV側 `r/e` を部分反映。`E` などは未対応
...
- [ ] target_type は 8/13+ を safety false 化済み。3/4 は heuristic を追加強化（HTML: root+close tag + root-before-close、MAIL: start-header + secondary-header-before-separator）、7 は printable+alpha。残は edge-case 詰めと更なる厳密化
...
5. [ ] PCRE flags / trigger prefix の残課題（`E`, maxshift厳密意味）
