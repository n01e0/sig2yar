...
- [x] `byte_comparison` は `i`(raw, 1/2/4/8byte) と non-raw `=/>/<`（基本は exact(e)、加えて strict subset として `width=1` の `!e` / `h+l`、`a`(auto) の `width<=2`）を条件式にlower済み。`h` base の数値トークンは hex 値として解釈（例: `=10` -> `0x10`）。offset は `strtol(...,0)` 相当へ寄せ、`0x..`/octal に加えて `+0x..`/`+010` などの明示 `+` 付き数値と、ClamAV互換の plain `0` / empty token（`#...`）を受理し、bare-hex（例: `>>0A`）は malformed 扱い。threshold は `0x..`/`+0x..`/`+010` を受理し、digit-only の leading-zero token は base-0 で octal 解釈（例: `=010` -> `8`）、invalid octal（例: `=08` / `=+08`）は malformed。decimal base では bare hex-alpha（例: `A0`）のみ strict false。unsupported ケース（non-raw `!e` の `width>1`、non-raw little-endian のうち `h+width=1` と `h+e+偶数width` 以外、`a`(auto) の `width>2`、表現不能値・`h` base 幅>18（`CLI_BCOMP_MAX_HEX_BLEN`）・decimal baseでbare hex-alpha閾値、comparison clause 3件以上（ClamAVは最大2件）、negative comparison value、rawの3/5/6/7byte および 9byte+・型幅超過閾値、矛盾した multi-clause、malformed byte_comparison format）は safety false に倒す（fallbackではなく厳密化）。2026-02-17: strict-false 主要経路に `clamav_unsupported` taxonomy tag を付与（追記146）。2026-02-17: strict subset として non-raw `width=1` の一部（`!e` / `h+l`）を strict support 化（追記147）。
...
- 2026-02-18 追記163: PCRE trigger prefix の distinct comparator を OR-only grouped subset まで strict support 拡張。
  - 変更: `pcre_trigger_expression_is_strictly_representable(...)` で `MultiGt/MultiLt` の許可条件を
    single-subsig 限定から OR-only grouped subset へ拡張。
  - 効果: `200,300:(0|1)>x,y/../` および `<x,y` が strict-false ではなく、
    `(#s0 + #s1)` + `y of (...)` 条件で strict lower される。
  - 維持: 非OR grouped（`&` 含む、nested count/distinct）は従来どおり `pcre_trigger_distinct_or_nested_count_unsupported` で strict-false。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` に OR-only support と non-OR strict-false を追加。
- 2026-02-18 追記162: A3 grouped distinct の OR-only strict subset を追加実装。
  - 変更: `MultiGt/MultiLt` grouped 条件のうち OR-only（`(a|b|...)>x,y` / `<x,y`）を
    `(#a + #b + ...)` と `y of ($a,$b,...)` の組み合わせへ strict lower。
  - 維持: 非OR grouped（`&` 含む、入れ子count/distinct等）は strict-false 維持（`requires OR-only string set` note）。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` に grouped OR support と grouped non-OR strict-false を追加。
...
- 2026-02-18 追記155: byte_comparison auto-base の strict subset を追加。
  ...
  - 維持: `a`(auto) の `width>2` は strict-false 維持（runtimeの 0x-prefix 分岐を未実装）。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` に `ae1` / `ae2`（support）と `ae3`（strict-false）の回帰を追加。
- 2026-02-18 追記154: byte_comparison offset の ClamAV互換 subset を追加。
  - 変更: `src/yara.rs` `parse_byte_comparison_offset(...)` で plain `0` / empty offset token（`#...`）を offset=0 として受理。
  - 根拠: ClamAV `matcher-byte-comp.c` は offset token 先頭が `0` または `\0` の場合を no-shift（0）として扱う。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` に `0(0#ib1#=65)` と `0(#ib1#=65)` の回帰を追加。
...
- 2026-02-17 追記147: `byte_comparison` non-raw strict-false の一部を strict support 化。
  - 変更: `src/yara.rs` の `lower_textual_byte_comparison_condition(...)` で `width=1` subset を許可。
    ...
    - non-raw little-endian は `h` base かつ `width=1` のとき strict support（ClamAV `matcher-byte-comp.c` の LE normalize が 1byte で no-op になる経路に合わせる）。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` に `db1` 非exactと `hle1` の match/non-match fixture を追加。
- 2026-02-17 追記146: `byte_comparison` strict-false 維持経路を `clamav_unsupported` tag で機械可読化。
  - 追加tag: `byte_comparison_clause_count_unsupported`, `byte_comparison_negative_value_unsupported`, `byte_comparison_contradictory_clauses`, `byte_comparison_trigger_unresolved`, `byte_comparison_trigger_non_string_reference`, `byte_comparison_nonraw_little_endian_unsupported`, `byte_comparison_nonraw_non_exact_unsupported`, `byte_comparison_nonraw_auto_base_unsupported`, `byte_comparison_decimal_bare_hex_alpha_unsupported`, `byte_comparison_nonraw_width_unsupported`, `byte_comparison_nonraw_hex_width_over_limit`, `byte_comparison_nonraw_unrepresentable_value`, `byte_comparison_raw_size_unsupported`, `byte_comparison_raw_threshold_out_of_range`, `byte_comparison_format_invalid`。
  - 変更: `src/yara.rs` の byte_comparison lowering false-path で tag を収集し、logical rule meta へ重複排除して出力。
  - テスト: `tests/yara_rule.rs` の byte_comparison strict-false fixture に tag 検証を追加。
...
- 2026-02-17 追記144: LDB/PCRE trigger prefix の distinct comparator（`>x,y`, `<x,y`）について、single-subsig subset のみ strict support 化。
  ...
  - 維持: grouped distinct（例: `(0|1)>x,y`）は strict-false 維持。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` に single-subsig distinct comparator の match/non-match fixture を追加、grouped distinct strict-false fixture は維持。
  - 備考: 追記143時点で strict-false としていた distinct comparator のうち、single-subsig 経路のみ本追記で supersede。
- 2026-02-17 追記143: LDB/PCRE trigger prefix の count-comparator subset（`=x`, `=x,y`, `>x`, `<x`）を strict-false から除外し、strict support 化。
  - 変更: `src/yara.rs` の trigger representable 判定を緩和し、count系のうち distinct を伴わない比較を許可。
  - 変更: distinct 系（`>x,y`, `<x,y`）は `distinct/nested-count operators unsupported for strict lowering` として strict-false 維持。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` の trigger-prefix count/gt/lt/range ケースを strict support 判定へ更新し、distinct 系は strict-false fixture を維持。
  - 備考: 追記70/95/96/97 で固定していた count 系 strict-false の一部を本追記で supersede。
- 2026-02-17 追記142: LDB/PCRE の range（maxshift）上 `r/re` を strict-false から除外し、window 条件で strict support 化。
  - 根拠: ClamAV `libclamav/matcher-pcre.c` では range 系（`adjshift != 0`）は `!rolling` でも anchored 分岐に入らず、`r` は判定窓の形を変えない。
  - 変更: `src/yara.rs` の `lower_pcre_offset_window_condition(...)` で range + rolling の early false を削除し、`e` ありは match-end window、`e` なしは match-start window に統一。
  - 適用範囲: absolute / `EP+/-` / `Sx+` / `SL+` / `SE` / `EOF-` の maxshift。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` の `...rolling_flag...` 系を strict-false 期待から window 条件期待へ更新し、PE fixture 上の match/non-match を固定。
  - 備考: 追記138/141 の「range+rolling を strict-false 維持」記述は本追記で supersede。
- 2026-02-17 追記141: LDB/PCRE の relative offset（`EP/Sx/SL/SE/EOF-`）上 non-`e` + maxshift を strict-false から除外し、strict support 化。
  - 根拠: ClamAV `libclamav/matcher-pcre.c` の non-encompass path。
    - maxshift あり (`adjshift != 0`) では buffer 全体走査後、`match_start <= adjshift` 条件で採否を決める。
    - この判定は offset prefix 共通で、`@ in [offset_start, offset_start+maxshift]` の start-window へ同型化可能。
  - 変更: `src/yara.rs` の range-window non-`e` lowering を `false` ではなく start-window 条件へ変更。
  - テスト: `tests/yara_rule.rs` / `tests/yara_compile.rs` に `EP+0,600:.../` と `SE1:.../` の positive coverage を追加。
  - 維持: `r` + maxshift（`/r` または `/re`）の relative path は runtime rolling semantics 未同型のため strict-false 維持。
