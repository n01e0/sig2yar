...
impl<'p> HduSignature<'p> {
    pub fn parse(sig: &'p str) -> Result<Self> {
        // ClamAV reference:
        // - docs: manual/Signatures (`hdu` is loaded in PUA mode)
        ...
        if parsed.hash_type != HashType::Md5 {
            return Err(anyhow!(
                "Invalid hdu signature: expected MD5 hash (32 hex chars)"
            ));
        }
        ...
        let size = match source {
            ...
            HashSource::Section { .. } => {
                return Err(anyhow!(
                    "Invalid hdu signature: expected file-hash format (section hash is not valid for .hdu)"
                ));
            }
        };
    ...
    pub fn to_ir(&self) -> ir::HduSignature {
        ir::HduSignature {
...
impl<'p> Display for HduSignature<'p> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", yara::render_hdu_signature(&self.to_ir()))
    }
}
...
mod tests {
    ...
    #[test]
    fn parse_basic_hdu_signature() {
        let sig = "44d88612fea8a8f36de82e1278abb02f:68:Eicar-Test-Signature";
        let parsed = HduSignature::parse(sig).unwrap();
    ...
    #[test]
    fn parse_hdu_with_wildcard_size_and_flevel() {
    ...
    #[test]
    fn parse_hdu_rejects_non_md5_hash() {
        let sig = "0059ee2322c3301263c8006fd780d7fe95a30572:1705472:Example";
        assert!(HduSignature::parse(sig).is_err());
    }

    #[test]
    fn parse_hdu_rejects_section_hash_format() {
