...
impl<'p> IgnSignature<'p> {
    pub fn parse(sig: &'p str) -> Result<Self> {
        ...
        if raw.trim().is_empty() {
            return Err(anyhow!(
                "Invalid ign signature: malformed record (empty signature name)"
            ));
        }
        ...
        if tokens.len() > 3 {
            return Err(anyhow!(
                "Invalid ign signature: malformed record (expected 1..3 ':'-separated fields)"
            ));
        }
    ...
    pub fn to_ir(&self) -> ir::IgnSignature {
        ir::IgnSignature {
...
impl<'p> Display for IgnSignature<'p> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", yara::render_ign_signature(&self.to_ir()))
    }
}

pub(crate) fn validate_md5(value: &str, db_type: &str) -> Result<()> {
    if value.len() != 32 || !value.chars().all(|c| c.is_ascii_hexdigit()) {
        return Err(anyhow!(
            "Invalid {db_type} signature: malformed md5 digest (expected 32 hex characters)"
        ));
    }
...
mod tests {
    ...
    #[test]
    fn parse_ign_name_only_record() {
        let sig = "Eicar-Test-Signature";
        let parsed = IgnSignature::parse(sig).unwrap();
    ...
    #[test]
    fn parse_ign_name_and_md5_record() {
    ...
    #[test]
    fn parse_ign_legacy_three_token_record() {
    ...
    #[test]
    fn parse_ign_rejects_empty_signature_name() {
        assert!(IgnSignature::parse(":bc356bae4c42f19a3de16e333ba3569c").is_err());
        assert!(IgnSignature::parse("legacy-repo:legacy-id:").is_err());
    }

    #[test]
    fn parse_ign_rejects_invalid_md5() {
        assert!(IgnSignature::parse("Eicar-Test-Signature:1234").is_err());
        assert!(
            IgnSignature::parse("Eicar-Test-Signature:zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz").is_err()
        );
    }

    #[test]
    fn parse_ign_rejects_too_many_tokens() {
        assert!(IgnSignature::parse("a:b:c:d").is_err());
    }
}
